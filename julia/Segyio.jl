...
function fopen(path :: AbstractString;
    ...
    sample_count = ccall( (:segy_samples, libsegyio), Cint,
                            (Ptr{Cchar},),
                            header )
    format = ccall( (:segy_format, libsegyio), Cint, (Ptr{Cchar},), header )
    trace_bsize = ccall( (:segy_trace_bsize, libsegyio), Cint,
                         (Cint,),
                         sample_count )
    ...
    err = ccall( (:segy_sorting, libsegyio), Cint,
                 (sgy, Cint, Cint, Ptr{Cint}, Clong, Cint),
    ...
    offset_count = Array(Cint, 1)
    err = ccall( (:segy_offsets, libsegyio), Cint,
                 (sgy, Cint, Cint, Cint, Ptr{Cint}, Clong, Cint),
    ...
    err = ccall( (:segy_lines_count, libsegyio), Cint,
                 (sgy, Cint, Cint, Cint, Cint, Ptr{Cint}, Ptr{Cint}, Clong, Cint),
    ...
    iline_length = ccall( (:segy_inline_length,    libsegyio), Cint,
                          (Cint,),
                          xline_count )
    xline_length = ccall( (:segy_crossline_length, libsegyio), Cint,
                          (Cint,),
                          iline_count )

    iline_stride = Array( Cint, 1 )
    ...
    ilines = Array(Cint, iline_count)
    err = ccall( (:segy_inline_indices, libsegyio), Cint,
                 (sgy, Cint, Cint, Cint, Cint, Cint, Ptr{Cint}, Clong, Cint),
...
function readtr!(fd:: SegyFile, i :: Int, buf)
    ...
    err = ccall( (:segy_readtrace, libsegyio), Cint,
                 (sgy, Cint, Ref{Cfloat}, Clong, Cint),
...
function readln!(ln :: Line,
    ...
    err = ccall( (:segy_read_line, libsegyio), Cint,
                 (sgy, Cint, Cint, Cint, Cint, Ref{Cfloat}, Clong, Cint),
...
function lntrace0(ln :: Line, i :: Integer, offset :: Integer)
    ...
    offset = offset - 1
    line_trace0 = Array(Cint, 1)
    err = ccall( (:segy_line_trace0, libsegyio), Cint,
                 (Cint, Cint, Cint, Cint, Ref{Cint}, Cint, Ptr{Cint}),
                 i,
                 length(ln.other),
                 ln.stride,
